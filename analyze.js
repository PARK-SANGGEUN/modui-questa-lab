...
`;
  ...
  const dbSummary = `
내부 DB 요약:
- 전체 사례 수: ${localDB.caseCount || 0}
- 현재 조건과 매칭된 사례 구조:
${(localDB.matchedCases||[]).slice(0,12).map((c,i)=>`${i+1}. [${c.university||""}/${c.major||""}/${c.subject||""}] ${c.topic||""} | 방법: ${c.method||""} | 구조: ${c.pattern||c.sourceStructure||""}`).join("\n")}
- 관련 도서/자료 관점:
${(localDB.relatedBooks||[]).slice(0,6).map((b,i)=>`${i+1}. ${b.title||""}(${b.author||""}) — ${b.use||""}`).join("\n")}
- 활용 가능한 탐구방법:
${(localDB.methodPool||[]).slice(0,8).map((m,i)=>`${i+1}. ${m.name||""}: 산출물 ${Array.isArray(m.outputs)?m.outputs.join(", "):""}`).join("\n")}
`;
...
[반드시 수행]
- 내부 사례 DB와 대학 학생부종합전형 가이드북, 합격생 세특 사례의 '구조'를 참고하되 제목과 문장은 새로 만든다.
...
가능하면 내부 사례 DB를 활용해 다음 구조를 참고하라. 검색 연결이 안 되더라도 네 지식으로 생성하라.
...
탐구 주제, 탐구 기간, 탐구 동기, 질문의 진화, 이론적 배경 및 교과 개념 연결, 참고 자료 및 선행 연구, 탐구 설계, 예비 탐구 및 수정 과정, 본 탐구 과정, 결과 및 해석, 한계점 및 개선 방향, 후속 탐구 방향, 세부학과 연결 및 진로 의미, 학생부 세특 예시, 면접 핵심 답변 소재.`;
  ...
  try{
    ...
    try{
      ...
      const repaired=await repairJSON({key,modelList,badText:result.text});
...
${badText}`;
  ...
  for(const model of modelList){
    try{
      ...
      if(!r.ok){
        ...
        data=await r.json();
        if(!r.ok){
          if(isModelError(data.error?.message)) continue;
          continue;
        }
      }
  ...
  return {error:"사용 가능한 Gemini 모델로 JSON 보정에 실패했습니다."};
}
...
function isModelError(msg){
  const s=String(msg||"").toLowerCase();
  return s.includes("not found")||s.includes("not supported for generatecontent")||s.includes("model")||s.includes("listmodels");
}

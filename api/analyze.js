export default async function handler(req, res) {
  ...
  try {
    // 안정성을 위해 기본은 검색 도구 없이 호출합니다. 검색 도구는 모델/JSON 모드와 충돌해 500 오류를 만들 수 있습니다.
    let result = await callGeminiWithFallback({ key, modelList, prompt, useSearch: false, mode: body.mode, jsonMode });
...
function buildDBContext(localDB) {
  ...
  const bookLines = books.map(b => `▸ 《${b.title || '도서명 없음'}》(${b.author || ''}) — ${b.use || ''}`).join('\n');

  // 선택 자료 DB가 없는 경우에도 서버가 죽지 않도록 안전 처리
  const bookInqSource = localDB.bookInqLines || localDB.bookInquiryLines || localDB.bookInquiryCases || localDB.bookCases || [];
  const mediaSource = localDB.mediaLines || localDB.mediaCases || localDB.openDataCases || [];
  const bookInqLines = Array.isArray(bookInqSource)
    ? bookInqSource.slice(0, 8).map((x, i) => typeof x === 'string' ? `${i + 1}. ${x}` : `${i + 1}. ${x.title || x.t || '독서 연계 탐구'} — ${x.summary || x.use || x.method || ''}`).join('\n')
    : String(bookInqSource || '');
  const mediaLines = Array.isArray(mediaSource)
    ? mediaSource.slice(0, 8).map((x, i) => typeof x === 'string' ? `${i + 1}. ${x}` : `${i + 1}. ${x.title || x.t || '미디어·오픈데이터 탐구'} — ${x.summary || x.use || x.method || ''}`).join('\n')
    : String(mediaSource || '');
...
function buildModelList(envModel) {
  const c = [envModel, 'gemini-2.0-flash', 'gemini-1.5-flash', 'gemini-1.5-flash-latest'].filter(Boolean);
  return [...new Set(c)];
}
  ...
  try {
    const payload = {
      ...
      generationConfig: {
        ...
        topP: 0.9,
        maxOutputTokens: mode === 'topics' || mode === 'transform' ? 8192 : 8192,
      },
    };
